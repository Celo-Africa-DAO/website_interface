...
export function CarouselSection() {
  ...
  return (
    <div className="bg-Celo-AD-primary -mt-20">
      <Section >
            ...
            <CarouselContent className="ml-0 flex space-x-0">
              {events.map((element) => (
                <CarouselItem key={element.id} className=" flex flex-col justify-between basis-[100%] md:basis-[35%]  p-0 m-0">
                  <Card className="flex flex-col justify-between h-full p-0 m-0 rounded-none  bg-transparent ">
                    <CardHeader className="flex flex-col gap-8">
                      <CardTitle className="font-gt-alpina-trial italic font-medium">{element.name}</CardTitle>
